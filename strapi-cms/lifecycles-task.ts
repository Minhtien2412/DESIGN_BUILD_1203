...
async function syncToBackend(event: string, data: any) {
  try {
    const response = await fetch(`${BACKEND_URL}/strapi-sync/tasks`, {
      method: "POST",
      headers: {
    ...
    console.log(`[Strapi->BE] Task ${event}: ${data.id} - Status: ${response.status}`);
  } catch (error) {
    console.error(`[Strapi->BE] Error syncing task:`, error);
  }
}
