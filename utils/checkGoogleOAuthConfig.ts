...
export function checkGoogleOAuthConfig() {
  ...
  if (config.webClientId || config.clientId) {
    console.log('   ✅ Configured:', (config.webClientId || config.clientId)?.substring(0, 30) + '...');
  } else {
  ...
  if (Platform.OS === 'android') {
    if (config.androidClientId) {
      console.log('   ✅ Configured:', config.androidClientId.substring(0, 30) + '...');
    } else {
  ...
  if (Platform.OS === 'ios') {
    if (config.iosClientId) {
      console.log('   ✅ Configured:', config.iosClientId.substring(0, 30) + '...');
    } else {
      console.log('   ⚠️  MISSING! Using web client ID as fallback');
      console.log('   ⚠️  Google Sign-in may not work properly');
  ...
  if (isExpoGo) {
    ...
    console.log('   ⚠️  Command: eas build --profile development --platform', Platform.OS);
  } else {
  ...
  if (Platform.OS === 'android' && androidPackage) {
    ...
    if (androidPackage === 'com.adminmarketingnx.APP_DESIGN_BUILD') {
      ...
      console.log('   ⚠️  Different from expected: com.adminmarketingnx.APP_DESIGN_BUILD');
