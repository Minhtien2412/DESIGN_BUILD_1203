...
export const MESSAGING_ENDPOINTS = {
  ...
  MESSAGES: (conversationId: string) =>
    `/api/v1/messages/conversations/${conversationId}/messages`, // GET
  SEND_MESSAGE: (conversationId: string) =>
    `/api/v1/messages/conversations/${conversationId}/messages`, // POST
...
export const NOTIFICATION_ENDPOINTS = {
  ...
  LIST: "/api/v1/notifications", // GET ?cursor=...&limit=20&category=...
  ...
  // Acknowledge receipt of real-time events (optional, "at-least-once" delivery)
  ACK: "/api/v1/notifications/ack", // POST { notificationIds, receivedAt }

  // User notification preferences
  SETTINGS: "/api/v1/notifications/settings", // GET / PUT
} as const;

// ===== JOBS (Pending / Progress) =====
export const JOB_ENDPOINTS = {
  STATUS: (jobId: string) => `/api/v1/jobs/${jobId}`, // GET - poll job status
  CANCEL: (jobId: string) => `/api/v1/jobs/${jobId}/cancel`, // POST - cancel a running job
  LIST_ACTIVE: "/api/v1/jobs/active", // GET - list user's active jobs
} as const;
...
export const COMPANY_ENDPOINTS = {
  LIST: "/api/v1/companies", // GET ?page=&limit=&type=&location=&search=
  DETAIL: (id: string) => `/api/v1/companies/${id}`, // GET
  BY_SLUG: (slug: string) => `/api/v1/companies/slug/${slug}`, // GET
  SEARCH: "/api/v1/companies/search", // POST
} as const;
