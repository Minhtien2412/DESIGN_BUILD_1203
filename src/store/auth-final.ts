...
// Create a state accessor that uses the get function internally
let stateAccessor: () => AuthState;

// Initialize the accessor using the store's subscribe mechanism
useAuthStore.subscribe((state) => {
  stateAccessor = () => state;
});

// Initialize with current state
stateAccessor = () => useAuthStore.getState ? useAuthStore.getState() : {} as AuthState;
...
export const authActions = {
  setSession: async (accessToken: string, refreshToken: string, user: User) => {
    return stateAccessor().setSession(accessToken, refreshToken, user);
  },
  clear: async () => {
    return stateAccessor().clear();
  },
  hydrate: async () => {
    return stateAccessor().hydrate();
  },
  setUser: (user: User) => {
    return stateAccessor().setUser(user);
  },
};