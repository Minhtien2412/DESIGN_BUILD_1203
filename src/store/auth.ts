...
export const getAuthState = (): AuthState => {
  return (useAuthStore as any).getState();
};

// Auth actions for external use
export const authActions = {
  setSession: async (accessToken: string, refreshToken: string, user: User) => {
    return (useAuthStore as any).getState().setSession(accessToken, refreshToken, user);
  },
  clear: async () => {
    return (useAuthStore as any).getState().clear();
  },
  hydrate: async () => {
    return (useAuthStore as any).getState().hydrate();
  },
  setUser: (user: User) => {
    return (useAuthStore as any).getState().setUser(user);
  },
};