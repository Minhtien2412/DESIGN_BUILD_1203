 ...
 * External Content: Pexels videos integration
 * @author AI Assistant
 * @date 03/01/2026
 * @updated 16/01/2026 - Thêm external content từ Pexels
 */
...
export default function VideoDiscoveryScreen() {
  ...
  // Fetch external videos from Pexels
  const { 
    ...
    category: selectedCategory === 'noithat' ? 'interior' 
      : selectedCategory === 'kientruc' ? 'villa' 
      : 'general',
    perPage: 12,
    enabled: true
  });
  
  // Fetch external photos for gallery
  const { 
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    Promise.all([refetchVideos(), refetchPhotos()]).finally(() => {
      setRefreshing(false);
    });
  }, [refetchVideos, refetchPhotos]);
    ...
    <View style={styles.container}>
      ...
      {!isSearchFocused && (
        ...
        >
          ...
          {/* External Photos from Pexels */}
          {externalPhotos.length > 0 && (
            <ExternalPhotoSection 
              photos={externalPhotos}
              title="📷 Ảnh tư liệu kiến trúc"
              subtitle="Ảnh chất lượng cao từ Pexels"
              isLoading={photosLoading}
            />
          )}
