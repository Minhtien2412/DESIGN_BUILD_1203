...
Write-Host "`n??  WARNINGS (Non-Critical)" -ForegroundColor Yellow
Write-Host "?????????????????????????????????????????????????????????????" -ForegroundColor Gray
...
Write-Host "  ??  4 deprecation warnings: Low priority" -ForegroundColor Yellow
Write-Host "  ??  2 minor errors: Ignorable" -ForegroundColor Yellow
