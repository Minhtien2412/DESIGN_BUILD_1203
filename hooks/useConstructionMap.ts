...
import {
    ...
    UserLeftEvent,
    ZoomChangedEvent,
} from "../services/websocket/construction-socket";
...
export function useConstructionMap({
  ...
  const loadProjectData = useCallback(async () => {
    try {
      ...
      console.log(
        `[useConstructionMap] Loaded ${loadedStages.length} stages, ${loadedTasks.length} tasks`,
      );
  ...
  const _setupEngineEvents = useCallback(
    (_engine: ConstructionMapEngine) => {
  ...
  const setupWebSocket = useCallback(() => {
    ...
    constructionSocket.onTaskStatusChanged(
      ({ taskId, status }: TaskStatusChangedEvent) => {
        console.log(
          `[useConstructionMap] Task status changed: ${taskId} -> ${status}`,
        );
        // TODO: Update engine
    ...
    constructionSocket.onUserJoined(
      ({ userId: joinedUserId, userName: joinedUserName }: UserJoinedEvent) => {
  ...
  const setupAutoSave = useCallback(() => {
    ...
    console.log(
      `[useConstructionMap] Auto-save enabled (${autoSaveInterval}ms)`,
    );
  ...
  const createStage = useCallback(
    async (data: Partial<Stage>): Promise<Stage | null> => {
      try {
        setSyncing(true);
        const res = await constructionMapApi.createStage({
          ...data,
          projectId,
        });
  ...
  const updateStage = useCallback(
    async (id: string, data: Partial<Stage>): Promise<Stage | null> => {
      try {
        setSyncing(true);
