...
export function useFeature(featureKey: string) {
  const [status, setStatus] = useState<FeatureStatus>(getFeatureStatus(featureKey));
  const [message, setMessage] = useState<string>(getFeatureFallbackMessage(featureKey));
 ...
 */
export function useNetworkSync(apiBaseUrl: string, getAuthToken: () => string | null) {
  ...
  const doSync = useCallback(async () => {
    ...
    try {
      ...
      if (results.documents > 0 || results.uploads > 0) {
        console.log(`[SYNC] Synced ${results.documents} docs, ${results.uploads} uploads`);
      }
    } catch (error) {
 ...
 */
export function useOfflineSupport(apiBaseUrl: string, getAuthToken: () => string | null) {
  const network = useNetworkSync(apiBaseUrl, getAuthToken);
  const fileCache = useFileCache();
  const uploader = useUpload();
  
  const hasOfflineData = network.pendingCount > 0 || fileCache.cachedFilesCount > 0;
