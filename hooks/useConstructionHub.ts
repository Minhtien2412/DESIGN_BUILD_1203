...
function _calculateMilestoneStatus(
...
export function useConstructionHub(): UseConstructionHubReturn {
  const [projects, setProjects] =
    useState<ConstructionProject[]>(MOCK_PROJECTS);
  const [activeProject, setActiveProject] =
    useState<ConstructionProject | null>(MOCK_PROJECTS[0]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [dataSource, setDataSource] = useState<"crm" | "mock">("mock");

  const calculateStats = useCallback(
    (projectList: ConstructionProject[]): ConstructionStats => {
      const totalBudget = projectList.reduce(
        (sum, p) => sum + p.budget.totalAmount,
        0,
      );
      const totalPaid = projectList.reduce(
        (sum, p) => sum + p.budget.paidAmount,
        0,
      );
      const avgProgress =
      ...
      return {
        ...
        activeProjects: projectList.filter((p) => p.status === "in_progress")
          .length,
        completedProjects: projectList.filter((p) => p.status === "completed")
          .length,
        onHoldProjects: projectList.filter((p) => p.status === "on_hold")
          .length,
        totalBudget,
        totalPaid,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedProjects: ConstructionProject[] = crmProjects.map(
        (project) => {
          const projectInvoices = crmInvoices.filter(
            (i) => i.project_id === parseInt(project.id),
          );
          const projectTasks = crmTasks.filter(
            (t) => t.rel_type === "project" && t.rel_id === project.id,
          );

          const totalBudget =
            parseFloat(project.project_cost || "0") ||
          ...
          const milestones: ConstructionMilestone[] = projectInvoices
            .slice(0, 3)
            .map((inv, idx) => ({
              id: `m${idx + 1}`,
          ...
          return {
            ...
            milestones:
              milestones.length > 0 ? milestones : MOCK_PROJECTS[0].milestones,
            tasks: {
      ...
      setDataSource("crm");
      console.log(
        "[useConstructionHub] Loaded from CRM:",
  ...
  const getProjectById = useCallback(
    (id: string): ConstructionProject | undefined => {
