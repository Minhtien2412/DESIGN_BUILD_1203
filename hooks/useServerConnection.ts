...
export function useServerConnection(autoCheck = true, intervalMs = 60000) {
  ...
  const checkServer = useCallback(async (config: typeof SERVERS[0]): Promise<ServerInfo> => {
    ...
    try {
      ...
      console.log(`[Health Check] Using API Key: ${ENV.API_KEY ? 'YES' : 'NO'}`);
      ...
      console.log(`[Health Check] ${config.name} - Status: ${response.status}, Time: ${responseTime}ms`);
      ...
      if (response.ok) {
        try {
          ...
          if (data.status === 'healthy') {
            ...
            console.warn(`[Health Check] ⚠️ ${config.name} - Unexpected status: ${data.status}`);
          }
        } catch (parseError) {
          console.error(`[Health Check] ❌ ${config.name} - JSON parse error:`, parseError);
        }
      } else {
        console.warn(`[Health Check] ⚠️ ${config.name} - HTTP ${response.status}`);
      }
      ...
      const responseTime = Date.now() - startTime;
      console.error(`[Health Check] ❌ ${config.name} - Error:`, error.message);
