...
export function useGoogleOAuth(options: UseGoogleOAuthOptions = {}) {
  ...
  const webClientId = (ENV as any).GOOGLE_WEB_CLIENT_ID || (ENV as any).GOOGLE_CLIENT_ID;
  ...
  useEffect(() => {
    if (!webClientId) {
      console.error('❌ [Google OAuth] Missing EXPO_PUBLIC_GOOGLE_CLIENT_ID in .env');
    }
    if (Platform.OS === 'android' && !androidClientId) {
      console.warn('⚠️ [Google OAuth] Missing EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID - using web client ID as fallback');
    }
    if (Platform.OS === 'ios' && !iosClientId) {
      console.warn('⚠️ [Google OAuth] Missing EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID - using web client ID as fallback');
    }
  ...
  useEffect(() => {
    ...
    if (response.type === 'success') {
      ...
      if (flow === 'auth-code') {
        ...
        console.log('[Google OAuth] Authorization Code:', result.code?.substring(0, 20) + '...');
      } else {
        // Implicit Flow
        result.accessToken = response.params.access_token;
        result.idToken = response.params.id_token;
        console.log('[Google OAuth] Access Token:', result.accessToken?.substring(0, 20) + '...');
      }
      ...
      const authError = new Error(response.error?.message || 'Đăng nhập Google thất bại');
      setError(authError);
      onError?.(authError);
      setLoading(false);
    }
  ...
  const signIn = async () => {
    if (!request) {
      const configError = new Error('Google OAuth chưa được cấu hình đúng. Vui lòng kiểm tra Client ID trong .env');
    ...
    try {
      ...
      const promptError = err instanceof Error ? err : new Error('Không thể bắt đầu đăng nhập Google');
