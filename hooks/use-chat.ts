...
export function useChat(options: UseChatOptions = {}): UseChatReturn {
  ...
  const loadStoredData = useCallback(async () => {
    try {
      ...
      if (currentChatId) {
        ...
        const { messages: apiMessages } = await chatAPIService.getMessages(currentChatId, { limit: 50 });
  ...
  useEffect(() => {
      ...
      setChatRooms(prev => prev.map(room => {
        if (room.id === message.chatId) {
          return {
            ...room,
            lastMessage: message,
            unreadCount: room.id === currentChatId ? room.unreadCount : room.unreadCount + 1,
    ...
    ChatService.on('onTyping', (status) => {
      if (status.chatId === currentChatId && status.userId !== user?.id?.toString()) {
        setTypingUsers(prev => {
  ...
  const sendMessage = useCallback(async (
    ...
    const result = await ChatService.sendMessage(currentChatId, content, options);
    return result !== null;
  }, [currentChatId]);

  // Delete message via API
  const deleteMessage = useCallback(async (messageId: string, forEveryone = false): Promise<boolean> => {
    if (!currentChatId) return false;
  ...
  const editMessage = useCallback(async (messageId: string, newContent: string): Promise<boolean> => {
    ...
    // Edit via API
    const updatedMessage = await chatAPIService.editMessage(messageId, newContent);
    if (updatedMessage) {
      // Update local state
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    try {
      ...
      const { messages: moreMessages, hasMore } = await chatAPIService.getMessages(currentChatId, {
        limit: 30,
      ...
      if (moreMessages.length > 0) {
        ...
        // Save to storage
        await ChatStorage.saveMessages(currentChatId, [...moreMessages, ...messages]);
      }
      
      console.log('[useChat] Loaded', moreMessages.length, 'more messages, hasMore:', hasMore);
    } catch (err) {
