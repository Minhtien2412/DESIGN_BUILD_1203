...
describe("aiService", () => {
  ...
  describe("sendChatMessage", () => {
    it("should send a chat message successfully", async () => {
      // Mock the raw API response (what apiFetch returns)
      ...
      // Expected result after aiService transforms it
      ...
      const result = await aiService.sendChatMessage("session-1", "Hi there");
  ...
  describe("estimateCost", () => {
    ...
    it("should handle errors when estimating cost", async () => {
      ...
      ).rejects.toThrow("Cost estimation error:");
    });
  });
  ...
  describe("suggestMaterials", () => {
    it("should suggest materials for a project", async () => {
      ...
      // Service expects response.suggestions
      mockApiFetch.mockResolvedValueOnce({ suggestions: mockSuggestions });
