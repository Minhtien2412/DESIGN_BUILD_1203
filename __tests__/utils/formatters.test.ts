...
describe("Currency Formatting", () => {
  ...
  test("formatCurrencyShort converts to billions/millions", () => {
    ...
    expect(formatCurrencyShort(500000)).toMatch(/500[.,]000.*₫/);
  });
...
describe("Number Formatting", () => {
  ...
  test("formatFileSize converts bytes correctly", () => {
    expect(formatFileSize(1024)).toMatch(/1(\.00)?\s*KB/);
    expect(formatFileSize(1048576)).toMatch(/1(\.00)?\s*MB/);
    expect(formatFileSize(0)).toMatch(/0\s*Bytes/);
  });
...
describe("Address Formatting", () => {
  test("formatAddress combines address parts", () => {
    ...
    expect(formatAddress(address)).toBe(
      "123 Nguyen Hue, P. Ben Nghe, Q1, TP. HCM"
    );
  });
