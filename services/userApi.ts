...
import type { CreateUserData, UpdateUserData, User, UserFilters, UserListResponse } from '@/types/core';
import { del, get, post, put } from './apiClient';

// Re-export profile API functions for convenience
export {
    deleteAvatar, getAvatarUrl, getProfile, isValidDateOfBirth, isValidVietnamesePhone, updateProfile, uploadAvatar
} from './profileApi';
...
export type { CreateUserData, UpdateUserData, User, UserFilters, UserListResponse };
 ...
 * });
 */
export async function updateUser(id: number, updates: UpdateUserData): Promise<User> {
...
export async function changePassword(
  ...
  console.log(`[UserAPI] 🔒 Changing password for user ${userId}...`);
  
  try {
    await post(`/users/${userId}/change-password`, {
      oldPassword,
      newPassword,
    });
