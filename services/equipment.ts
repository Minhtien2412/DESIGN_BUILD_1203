...
export async function getEquipment(
  ...
  if (params.ownershipType)
    queryParams.append("ownershipType", params.ownershipType);
  if (params.assignedTo) queryParams.append("assignedTo", params.assignedTo);
  if (params.search) queryParams.append("search", params.search);

  const res = await apiFetch(`/equipment?${queryParams.toString()}`);
  // API may return { data: [...] }, { equipment: [...] }, or a direct array
  if (Array.isArray(res)) return res;
  if (res && Array.isArray(res.data)) return res.data;
  if (res && Array.isArray(res.equipment)) return res.equipment;
  return [];
}
...
export async function getEquipmentStats(
  params: GetEquipmentStatsParams,
...
export async function getMaintenanceCostReport(
  ...
  return apiFetch(
    `/equipment/reports/maintenance-cost?${queryParams.toString()}`,
  );
}
...
export async function getFuelConsumptionReport(
  ...
  return apiFetch(
    `/equipment/reports/fuel-consumption?${queryParams.toString()}`,
  );
}
