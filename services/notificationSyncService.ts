...
export interface UnifiedNotification {
  ...
  type: 'INFO' | 'SUCCESS' | 'WARNING' | 'ERROR' | 'TASK' | 'MESSAGE' | 'PAYMENT' | 'PROJECT' | 'TICKET';
  title: string;
  message: string;
  isRead: boolean;
...
const MOCK_NOTIFICATIONS: UnifiedNotification[] = [
  {
    ...
    message: 'Bạn được giao công việc thiết kế mặt bằng tầng 1 cho dự án Villa Premium. Deadline: 15/01/2026',
    isRead: false,
    priority: 'HIGH',
    relatedId: '101',
    relatedType: 'task',
  ...
  },
  {
    ...
    message: 'Anh ơi, em cần xác nhận lại thiết kế nội thất phòng khách trước khi chuyển sang giai đoạn tiếp theo nhé!',
    isRead: false,
    priority: 'MEDIUM',
    relatedId: '45',
    relatedType: 'chat',
  ...
  },
  {
    ...
    message: 'Dự án "Biệt thự Phú Quốc" đã được cập nhật tiến độ lên 75%. Giai đoạn xây thô hoàn thành.',
    isRead: false,
    priority: 'MEDIUM',
    relatedId: '12',
    relatedType: 'project',
  ...
  },
  {
    ...
    message: 'Đã nhận thanh toán 150.000.000 VND cho giai đoạn 2 dự án Villa Premium. Mã giao dịch: PAY-2026-001234',
    isRead: false,
    priority: 'LOW',
    relatedId: '33',
    relatedType: 'payment',
  ...
  },
  {
    ...
    message: 'Công việc "Khảo sát mặt bằng" đã được đánh dấu hoàn thành bởi team kỹ thuật.',
    isRead: true,
    priority: 'LOW',
    relatedId: '88',
    relatedType: 'task',
  ...
  },
  {
    ...
    message: 'Công việc "Bản vẽ kiến trúc tầng 1" còn 2 ngày nữa là đến hạn. Vui lòng kiểm tra và hoàn thành đúng tiến độ.',
    isRead: false,
    priority: 'URGENT',
    relatedId: '102',
    relatedType: 'task',
  ...
  },
  {
    ...
    message: 'Trần Thị B đã tham gia dự án "Căn hộ Sunrise City" với vai trò Kỹ sư kết cấu.',
    isRead: true,
    priority: 'LOW',
    relatedId: '15',
    relatedType: 'project',
  ...
  },
  {
    ...
    message: 'Khách hàng Lê Văn C đã gửi yêu cầu hỗ trợ về vấn đề đổi màu sơn tường. Độ ưu tiên: Cao',
    isRead: false,
    priority: 'HIGH',
    relatedId: '201',
    relatedType: 'ticket',
...
async function fetchCRMTasks(contactId?: string): Promise<CRMTask[]> {
  try {
    // Thử endpoint tasks
    const response = await fetchCRM<{ status: boolean; data?: CRMTask[] }>('tasks');
    ...
    if (contactId) {
      const contactTasks = await fetchCRM<CRMTask[]>(`tasks?contact_id=${contactId}`);
      return contactTasks || [];
    }
 ...
 */
async function fetchCRMProjectUpdates(projectId: string): Promise<CRMProjectUpdate[]> {
  try {
    const response = await fetchCRM<{ status: boolean; data?: CRMProjectUpdate[] }>(
 ...
 */
async function fetchCRMActivities(relType?: string, relId?: string): Promise<CRMActivity[]> {
  try {
    ...
    const response = await fetchCRM<{ status: boolean; data?: CRMActivity[] }>(endpoint);
...
function convertCRMTask(task: CRMTask): UnifiedNotification {
  const isOverdue = task.duedate && new Date(task.duedate) < new Date() && task.status !== '5';
...
export const NotificationSyncService = {
  ...
  syncAll: async (options?: {
    ...
    try {
      ...
      try {
        ...
        console.log(`[NotificationSync] App notifications: ${appNotifications.length}`);
      } catch (err) {
        console.warn('[NotificationSync] Failed to fetch app notifications:', err);
      }
      ...
      if (options?.projectIds) {
        for (const projectId of options.projectIds) {
          ...
          activities.slice(0, 10).forEach(activity => { // Limit 10 per project
      ...
      await setItem(STORAGE_KEYS.CRM_NOTIFICATIONS, JSON.stringify(finalNotifications));
      await setItem(STORAGE_KEYS.LAST_SYNC, syncedAt);
      await setItem(STORAGE_KEYS.SYNC_STATUS, 'success');
      
      const crmCount = finalNotifications.filter(n => n.source === 'CRM').length;
      const appCount = finalNotifications.filter(n => n.source === 'APP').length;
      
      console.log(`[NotificationSync] Completed in ${Date.now() - startTime}ms`);
      console.log(`[NotificationSync] Total: ${finalNotifications.length} (CRM: ${crmCount}, App: ${appCount})`);
      ...
      await setItem(STORAGE_KEYS.CRM_NOTIFICATIONS, JSON.stringify(MOCK_NOTIFICATIONS));
      await setItem(STORAGE_KEYS.SYNC_STATUS, 'demo');
      
      const crmMockCount = MOCK_NOTIFICATIONS.filter(n => n.source === 'CRM').length;
      const appMockCount = MOCK_NOTIFICATIONS.filter(n => n.source === 'APP').length;
  ...
  getAll: async (): Promise<UnifiedNotification[]> => {
    try {
      const cached = await getItem(STORAGE_KEYS.CRM_NOTIFICATIONS);
      if (cached) {
      ...
      console.error('[NotificationSync] Failed to get cached notifications:', err);
    }
   ...
   */
  getUnreadCount: async (): Promise<{ total: number; crm: number; app: number }> => {
  ...
  markAsRead: async (notificationId: string): Promise<void> => {
    ...
    await setItem(STORAGE_KEYS.CRM_NOTIFICATIONS, JSON.stringify(updated));
    ...
    if (notificationId.startsWith('app_')) {
      ...
      try {
        ...
        console.warn('[NotificationSync] Failed to mark app notification as read:', err);
      }
    }
  ...
  markAllAsRead: async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[NotificationSync] Failed to mark all app notifications as read:', err);
    }
  },
  ...
  clearCache: async (): Promise<void> => {
    await setItem(STORAGE_KEYS.CRM_NOTIFICATIONS, '[]');
