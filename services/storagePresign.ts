...
export async function requestPresignUpload(request: PresignRequest): Promise<PresignResponse> {
  // Map client request to server expectations
  const filename = `upload${request.ext || ''}`;
  const body = {
    filename,
    contentType: request.mime,
    prefix: request.folder || 'uploads',
    // expiresSeconds: undefined, // use server default
  };

  const resp = await apiFetch<RawPresignResponse>('/api/uploads/presign', {
    method: 'POST',
    data: body,
    timeoutMs: 20000,
  });
  ...
  if (typeof resp === 'object' && resp && 'success' in resp) {
    ...
    const method = String(data.method || request.method || 'put').toLowerCase() as UploadMethod;
    if (method === 'post') {
...
export async function uploadToPresignedUrl({ presign, fileUri, mime, fileName, signal, onProgress }: UploadParams) {
  ...
  if (Platform.OS !== 'web' && typeof (FileSystem as any).createUploadResumable === 'function') {
    ...
    form.append('file', { uri: fileUri, name: fileName, type: mime || 'application/octet-stream' } as any);
    const response = await fetch(presign.url, { method: 'POST', body: form, signal });
  ...
  const response = await fetch(presign.url, { method: 'PUT', headers, body: await (await fetch(fileUri)).blob(), signal });
  if (!response.ok) throw new Error(`Upload thất bại (${response.status})`);
}
