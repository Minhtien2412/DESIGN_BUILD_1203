...
export const createWorker = async (data: CreateWorkerRequest): Promise<Worker> => {
...
export const getAttendance = async (attendanceId: string): Promise<Attendance> => {
...
export const assignShift = async (data: AssignShiftRequest): Promise<ShiftAssignment> => {
...
export const getLeaveRequest = async (leaveRequestId: string): Promise<LeaveRequest> => {
...
export const cancelLeaveRequest = async (leaveRequestId: string): Promise<LeaveRequest> => {
  return apiFetch(`${BASE_URL}/leave-requests/${leaveRequestId}/cancel`, {
    method: 'POST',
  });
};

export const getWorkerLeaveRequests = async (workerId: string): Promise<LeaveRequest[]> => {
...
export const getWorkerSummary = async (projectId?: string): Promise<WorkerSummary> => {
  const params = projectId ? `?projectId=${projectId}` : '';
...
export const getAttendanceSummary = async (
  ...
  if (projectId) params.append('projectId', projectId);
  return apiFetch(`${BASE_URL}/reports/attendance-summary?${params.toString()}`);
};
...
export const exportAttendance = async (
  ...
  if (projectId) params.append('projectId', projectId);
  return apiFetch(`${BASE_URL}/reports/export-attendance?${params.toString()}`, {
    headers: { Accept: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' },
  });
};
