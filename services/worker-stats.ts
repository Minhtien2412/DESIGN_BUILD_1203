...
import type { RegisterWorkerRequest, WorkerRegistration, WorkerStatsResponse } from '@/types/worker-stats';
import { WorkerType } from '@/types/worker-stats';
import { apiFetch } from './api';

const WORKER_STATS_ENDPOINT = '/api/workers/stats';
const WORKER_REGISTRATION_ENDPOINT = '/api/workers/register';
...
export async function getWorkerStats(): Promise<WorkerStatsResponse> {
  try {
    ...
    console.error('[WorkerStatsService] Failed to fetch worker stats:', error);
 ...
 */
export async function registerWorker(data: RegisterWorkerRequest): Promise<WorkerRegistration> {
  try {
