...
const WIDGET_CONFIGS: WidgetConfig[] = [
  ...
  },
  {
    ...
    getValue: (stats) =>
      stats?.total_budget
        ? `${(stats.total_budget / 1000000).toFixed(0)}M`
        : "0",
    color: "#0D9488",
    route: "/budget",
  },
...
function StatCard({
  ...
  return (
    <TouchableOpacity
      style={[
        styles.statCard,
        { backgroundColor: colors.surface, borderLeftColor: widget.color },
      ]}
...
export default function AdminDashboardRBAC() {
  ...
  const canViewProjects = usePermission(
    PermissionModule.PROJECTS,
    PermissionAction.VIEW,
  );

  const { stats, recentProjects, recentActivities, loading, error, refresh } =
    useDashboard();
