...
export default function AdminProductsScreen() {
  ...
  const { data: productsData, loading, error, execute } = useApiCall(
  ...
  const handleUpdateStatus = async (product: Product, status: 'APPROVED' | 'REJECTED') => {
    try {
      await productService.updateStatus(product.id, { status });
      Alert.alert('Thành công', `Đã ${status === 'APPROVED' ? 'duyệt' : 'từ chối'} sản phẩm`);
      loadProducts();
    } catch (error) {
    ...
    <View style={styles.productCard}>
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.statusBadge}>
          ...
          >
            {item.status === ProductStatus.APPROVED ? 'Đã duyệt' : 
             item.status === ProductStatus.PENDING ? 'Chờ duyệt' : 'Từ chối'}
          </Text>
        </View>
      </View>
  ...
  return (
    <SafeScrollView style={styles.container}>
      ...
      <View style={styles.modeRow}>
        <TouchableOpacity
          style={[styles.modeBtn, viewMode === 'dashboard' && styles.modeBtnActive]}
          onPress={() => setViewMode('dashboard')}
        >
          <Ionicons name="grid-outline" size={18} color={viewMode === 'dashboard' ? '#fff' : '#666'} />
          <Text style={[styles.modeText, viewMode === 'dashboard' && styles.modeTextActive]}>Dashboard</Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          ...
          <Text style={[styles.modeText, viewMode === 'list' && styles.modeTextActive]}>Danh sách</Text>
        </TouchableOpacity>
      </View>
        ...
        <>
          ...
          <View style={styles.quickSection}>
            <View style={styles.quickHeader}>
              <Text style={styles.quickTitle}>Chờ duyệt ({pendingProducts.length})</Text>
              <TouchableOpacity
                onPress={() => {
                  setViewMode('list');
            ...
            {pendingProducts.length > 0 ? (
              <View style={styles.quickList}>
                {pendingProducts.slice(0, 6).map((p) => (
                  <View key={p.id.toString()}>{renderProduct({ item: p })}</View>
                ))}
              </View>
            ) : (
        ...
        <>
          ...
          <View style={styles.stats}>
            <Text style={styles.statsText}>
              Tổng: <Text style={styles.statsNumber}>{products.length}</Text> sản phẩm
            </Text>
            <Text style={styles.statsText}>
              Chờ duyệt: <Text style={styles.statsNumber}>{pendingProducts.length}</Text>
            </Text>
          </View>
