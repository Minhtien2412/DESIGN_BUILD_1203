...
const nearbyItems = [
  ...
  },
  {
    ...
    isOpen: true,
    image: 'https://via.placeholder.com/100',
    categories: ['Bồn cầu', 'Lavabo', 'Vòi sen'],
  },
  {
    id: '3',
    type: 'warehouse',
        ...
        <View style={styles.itemHeader}>
          ...
          {'isOpen' in item ? (
            <View style={[styles.statusBadge, { backgroundColor: item.isOpen ? '#E8F5E9' : '#FFEBEE' }]}>
              <Text style={{ color: item.isOpen ? '#4CAF50' : '#F44336', fontSize: 10 }}>
                {item.isOpen ? 'Đang mở' : 'Đã đóng'}
              </Text>
            </View>
          ) : (
            <View style={[styles.statusBadge, { backgroundColor: item.isAvailable ? '#E8F5E9' : '#FFEBEE' }]}>
              <Text style={{ color: item.isAvailable ? '#4CAF50' : '#F44336', fontSize: 10 }}>
...
const styles = StyleSheet.create({
  ...
  infoRow: { flexDirection: 'row', alignItems: 'center', marginTop: 6, gap: 12 },
  ...
  tag: { backgroundColor: '#f0f0f0', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
