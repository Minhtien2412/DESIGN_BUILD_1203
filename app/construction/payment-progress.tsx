...
function MilestoneCard({ milestone }: { milestone: PaymentMilestone }) {
  const getStatusColor = () => {
    switch (milestone.status) {
      case "paid":
        return { bg: "#F0FDF4", border: "#0D9488", text: "#0D9488" };
      case "approved":
        return { bg: "#FEF3C7", border: "#0D9488", text: "#0D9488" };
      default:
    ...
    >
      <View style={styles.milestoneHeader}>
        <View style={{ flex: 1, marginRight: 8 }}>
          <Text style={[styles.milestoneTitle, { color: statusColors.text }]}>
            {milestone.title}
          </Text>
          {milestone.status === "paid" && (
            ...
            >
              ...
              <Text
                style={{ fontSize: 11, color: "#0D9488", fontWeight: "500" }}
              >
                Đã thanh toán
              </Text>
            </View>
      ...
      <Text style={[styles.milestonePercentage, { color: statusColors.text }]}>
      ...
      {milestone.approvedBy && (
        <View style={styles.approvalInfo}>
          ...
          <View style={{ alignItems: "flex-end" }}>
            <Text style={[styles.approvalValue, { color: statusColors.text }]}>
              {milestone.approvedBy}
            </Text>
            <Text style={styles.approvalDate}>
              Ngày {milestone.approvedDate}
            </Text>
            {milestone.accountantApproved && (
              <Text
                style={[
                  styles.accountantApproved,
  ...
  return (
    ...
    >
      ...
      <View style={styles.phaseRight}>
        ...
        {hasContent && (
          <Ionicons name="chevron-forward" size={16} color="#1F1F39" />
        )}
      </View>
    </TouchableOpacity>
  );
...
export default function PaymentProgressScreen() {
  ...
  const [data, setData] = useState<PaymentProgress>(MOCK_PAYMENT_DATA);
  ...
  const loadPaymentData = useCallback(async () => {
    try {
      ...
      const budgetResp =
        await budgetTrackingService.getProjectBudget(projectIdNum);
      const budget: ApiBudget = (budgetResp as any)?.data ?? budgetResp;
      if (budget && budget.id) {
        // Load budget lines as phases
        const linesResp = await budgetTrackingService.getBudgetLines(budget.id);
        const lines: ApiBudgetLine[] =
          (linesResp as any)?.data ?? linesResp ?? [];

        if (Array.isArray(lines) && lines.length > 0) {
          const phases: PaymentPhase[] = lines.map(
            (l: ApiBudgetLine, idx: number) => ({
              id: `line-${l.id}`,
              name: l.description || l.category || `Giai đoạn ${idx + 1}`,
              totalAmount: l.plannedAmount,
              status:
                l.actualAmount >= l.plannedAmount
                  ? ("completed" as const)
                  : l.actualAmount > 0
              ...
              milestones: [
                {
                  id: `ms-${l.id}`,
                  title: l.description || l.category,
                  amount: l.plannedAmount,
                  percentage:
                    l.plannedAmount > 0
                      ? Math.round((l.actualAmount / l.plannedAmount) * 100)
                      : 0,
                  status:
                    l.actualAmount >= l.plannedAmount
                      ? ("paid" as const)
          ...
          const totalAmount = budget.totalBudget;
          const paidAmount = budget.totalSpent;
          ...
          console.log(
            "[PaymentProgress] Loaded from API:",
            phases.length,
            "phases",
          );
        }
      ...
      console.log("[PaymentProgress] API error, using mock data:", error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      >
        ...
        >
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={[styles.summaryPercentage, { color: "#13B157" }]}>
            ...
            <View key={phase.id} style={styles.activePhaseSection}>
              <View style={styles.phaseHeader}>
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.activePhaseTotalAmount,
                      {
                        color:
                          phase.status === "completed" ? "#0D9488" : "#13B157",
                      },
                    ]}
                  >
                    {formatCurrency(phase.totalAmount)}
