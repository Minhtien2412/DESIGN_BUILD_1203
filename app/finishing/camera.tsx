...
const WORKERS: Worker[] = [
  {
    id: 1,
    name: 'Đội Lắp Camera Chuyên Nghiệp',
    rating: 4.9,
    reviews: 267,
    location: 'TP.HCM',
    experience: 16,
    price: '800.000₫',
    priceUnit: '/ camera',
    specialties: ['Camera IP', 'Camera Wifi', 'CCTV', 'Camera 4K', 'Hệ thống giám sát'],
    teamSize: 4,
    completedProjects: 523,
    avatar: '📹',
    featured: true,
    availability: 'available',
    installationFee: '+ 300k lắp đặt',
  },
  {
    id: 2,
    name: 'Camera An Ninh Hà Nội',
    rating: 4.8,
    reviews: 234,
    location: 'Hà Nội',
    experience: 14,
    price: '750.000₫',
    priceUnit: '/ camera',
    specialties: ['Camera IP', 'Camera có dây', 'Báo động', 'Kiểm soát ra vào'],
    teamSize: 3,
    completedProjects: 467,
    avatar: '🎥',
    featured: true,
    availability: 'available',
    installationFee: '+ 280k lắp đặt',
  },
  {
    id: 3,
    name: 'Lắp Camera Miền Trung',
    rating: 4.7,
    reviews: 198,
    location: 'Đà Nẵng',
    experience: 12,
    price: '700.000₫',
    priceUnit: '/ camera',
    specialties: ['Camera Wifi', 'Camera ngoài trời', 'Đầu ghi hình', 'Bảo trì'],
    teamSize: 3,
    completedProjects: 389,
    avatar: '📷',
    availability: 'available',
    installationFee: '+ 250k lắp đặt',
  },
  {
    id: 4,
    name: 'Camera Giám Sát Phương Nam',
    rating: 4.6,
    reviews: 176,
    location: 'Cần Thơ',
    experience: 10,
    price: '650.000₫',
    priceUnit: '/ camera',
    specialties: ['Camera IP', 'Camera Wifi', 'CCTV', 'Sửa chữa'],
    teamSize: 2,
    completedProjects: 312,
    avatar: '📹',
    availability: 'busy',
    installationFee: '+ 220k lắp đặt',
  },
];

const LOCATIONS = ['Tất cả', 'TP.HCM', 'Hà Nội', 'Đà Nẵng', 'Cần Thơ', 'Khác'];
const CAMERA_TYPES = ['Tất cả', 'Camera IP', 'Camera Wifi', 'CCTV', 'Camera có dây', 'Camera 4K'];

export default function CameraInstallerScreen() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedLocation, setSelectedLocation] = useState('Tất cả');
  const [selectedCameraType, setSelectedCameraType] = useState('Tất cả');
  ...
  const filteredWorkers = WORKERS.filter((worker) => {
    ...
    const matchesCameraType =
      selectedCameraType === 'Tất cả' ||
      worker.specialties.some((s) => s.includes(selectedCameraType));

    return matchesSearch && matchesLocation && matchesCameraType;
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Lắp camera</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder="Tìm thợ lắp camera..."
            value={searchQuery}
        ...
        {/* Camera Type Filter */}
        <View style={styles.filterSection}>
          <Text style={styles.filterLabel}>Loại camera</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            {CAMERA_TYPES.map((type) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterChip,
                  selectedCameraType === type && styles.filterChipActive,
                ]}
                onPress={() => setSelectedCameraType(type)}
              >
                <Text
                  style={[
                    styles.filterChipText,
                    selectedCameraType === type && styles.filterChipTextActive,
                  ]}
                >
                  {type}
                </Text>
        ...
        <View style={styles.resultsHeader}>
          <Text style={styles.resultsCount}>
            {filteredWorkers.length} thợ lắp camera
          </Text>
          {(selectedLocation !== 'Tất cả' || selectedCameraType !== 'Tất cả' || searchQuery) && (
        ...
        {filteredWorkers.length > 0 ? (
            ...
            <View key={worker.id} style={styles.workerCard}>
              ...
              <View style={styles.priceRow}>
                <View>
                  ...
                  {worker.installationFee && (
                    <Text style={styles.priceAlt}>{worker.installationFee}</Text>
                  )}
                </View>
                <TouchableOpacity
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>Không tìm thấy thợ lắp camera</Text>
        ...
        >
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Đặt lịch lắp đặt</Text>
            ...
            <ScrollView style={styles.modalForm} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.formGroup}>
                <Text style={styles.formLabel}>Địa chỉ lắp đặt *</Text>
                <TextInput
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  style={styles.formInput}
                  placeholder="VD: Camera IP Wifi 2MP"
                  value={bookingForm.cameraType}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, cameraType: text })}
                />
              </View>
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  style={styles.formInput}
                  placeholder="Nhập số lượng camera cần lắp"
                  value={bookingForm.quantity}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, quantity: text })}
                  keyboardType="numeric"
                />
              </View>

              <View style={styles.formGroup}>
                <Text style={styles.formLabel}>Khu vực giám sát</Text>
                <TextInput
                  style={[styles.formInput, styles.formTextArea]}
                  placeholder="VD: Cổng chính, sân sau, phòng khách..."
                  value={bookingForm.coverage}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, coverage: text })}
                  multiline
                  numberOfLines={3}
                />
              </View>
