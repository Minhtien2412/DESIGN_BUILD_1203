...
const WORKERS: Worker[] = [
  {
    ...
    price: '2.500.000₫',
    priceUnit: '/ cánh',
    specialties: ['Cửa gỗ', 'Cửa nhôm', 'Cửa kính', 'Cửa tự động'],
    teamSize: 4,
    completedProjects: 428,
    avatar: '🚪',
    featured: true,
    availability: 'available',
    installationFee: '+ 500k lắp đặt',
  },
  {
    id: 2,
    name: 'Thợ Làm Cửa Chuyên Nghiệp',
    rating: 4.8,
    reviews: 187,
    location: 'Hà Nội',
    experience: 16,
    price: '2.200.000₫',
    priceUnit: '/ cánh',
    specialties: ['Cửa gỗ', 'Cửa nhôm kính', 'Cửa cuốn', 'Cửa sắt'],
    teamSize: 3,
    completedProjects: 392,
    avatar: '🔧',
    featured: true,
    availability: 'available',
    installationFee: '+ 450k lắp đặt',
  },
  {
    id: 3,
    name: 'Làm Cửa Miền Trung',
    rating: 4.7,
    reviews: 164,
    location: 'Đà Nẵng',
    experience: 14,
    price: '2.000.000₫',
    priceUnit: '/ cánh',
    specialties: ['Cửa nhôm', 'Cửa kính', 'Cửa lưới', 'Cửa phòng'],
    teamSize: 3,
    completedProjects: 318,
    avatar: '🏗️',
    availability: 'available',
    installationFee: '+ 400k lắp đặt',
  },
  {
    id: 4,
    name: 'Thợ Cửa Phương Nam',
    rating: 4.6,
    reviews: 142,
    location: 'Cần Thơ',
    experience: 12,
    price: '1.800.000₫',
    priceUnit: '/ cánh',
    specialties: ['Cửa gỗ công nghiệp', 'Cửa nhựa', 'Cửa lưới', 'Sửa chữa'],
    teamSize: 2,
    completedProjects: 267,
    avatar: '🚪',
    availability: 'busy',
    installationFee: '+ 350k lắp đặt',
  },
];

const LOCATIONS = ['Tất cả', 'TP.HCM', 'Hà Nội', 'Đà Nẵng', 'Cần Thơ', 'Khác'];
const DOOR_TYPES = ['Tất cả', 'Cửa gỗ', 'Cửa nhôm', 'Cửa kính', 'Cửa cuốn', 'Cửa sắt'];

export default function DoorInstallerScreen() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedLocation, setSelectedLocation] = useState('Tất cả');
  const [selectedDoorType, setSelectedDoorType] = useState('Tất cả');
  ...
  const filteredWorkers = WORKERS.filter((worker) => {
    ...
    const matchesDoorType =
      selectedDoorType === 'Tất cả' ||
      worker.specialties.some((s) => s.includes(selectedDoorType));

    return matchesSearch && matchesLocation && matchesDoorType;
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Thợ làm cửa</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Door Type Filter */}
        <View style={styles.filterSection}>
          <Text style={styles.filterLabel}>Loại cửa</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            {DOOR_TYPES.map((type) => (
              <TouchableOpacity
                key={type}
                style={[
                  styles.filterChip,
                  selectedDoorType === type && styles.filterChipActive,
                ]}
                onPress={() => setSelectedDoorType(type)}
              >
                <Text
                  style={[
                    styles.filterChipText,
                    selectedDoorType === type && styles.filterChipTextActive,
                  ]}
                >
                  {type}
                </Text>
        ...
        <View style={styles.resultsHeader}>
          <Text style={styles.resultsCount}>
            {filteredWorkers.length} thợ làm cửa
          </Text>
          {(selectedLocation !== 'Tất cả' || selectedDoorType !== 'Tất cả' || searchQuery) && (
        ...
        {filteredWorkers.length > 0 ? (
          ...
          <View style={styles.emptyState}>
            <Ionicons name="exit-outline" size={64} color="#ccc" />
            <Text style={styles.emptyText}>Không tìm thấy thợ làm cửa</Text>
        ...
        <View style={styles.infoBanner}>
          ...
          <Text style={styles.infoBannerText}>
            Thi công chuyên nghiệp • Bảo hành 24 tháng
          </Text>
        </View>
      </ScrollView>
        ...
        >
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalForm} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.formGroup}>
                <Text style={styles.formLabel}>Loại cửa</Text>
                <TextInput
                  style={styles.formInput}
                  placeholder="VD: Cửa gỗ 4 cánh"
                  value={bookingForm.doorType}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, doorType: text })}
                />
              </View>
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  style={styles.formInput}
                  placeholder="Nhập số lượng cửa cần làm"
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  style={styles.formInput}
                  placeholder="VD: 2m x 1.2m"
                  value={bookingForm.dimensions}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, dimensions: text })}
                />
              </View>
