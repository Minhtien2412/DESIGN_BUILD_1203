...
const WORKERS: Worker[] = [
  {
    ...
    price: '650.000₫',
    priceUnit: '/ mét',
    specialties: ['Lan can inox', 'Lan can kính', 'Cầu thang', 'Ban công'],
    teamSize: 4,
  ...
  {
    id: 2,
    name: 'Thợ Lan Can Chuyên Nghiệp',
    rating: 4.8,
    reviews: 198,
    location: 'Hà Nội',
    experience: 15,
    price: '600.000₫',
    priceUnit: '/ mét',
    specialties: ['Lan can sắt', 'Lan can gỗ', 'Cầu thang xoắn', 'Tay vịn'],
    teamSize: 3,
  ...
  },
  {
    ...
    price: '550.000₫',
    priceUnit: '/ mét',
    specialties: ['Lan can inox', 'Lan can nhôm', 'Ban công', 'Sân thượng'],
    teamSize: 3,
    completedProjects: 348,
    avatar: '⚙️',
    availability: 'available',
  },
  {
    id: 4,
    name: 'Thợ Lan Can Phương Nam',
    rating: 4.6,
    reviews: 154,
    location: 'Cần Thơ',
    experience: 11,
    price: '500.000₫',
    priceUnit: '/ mét',
    specialties: ['Lan can sắt mỹ thuật', 'Tay vịn gỗ', 'Cầu thang', 'Sửa chữa'],
    teamSize: 2,
...
const RAILING_TYPES = ['Tất cả', 'Lan can inox', 'Lan can kính', 'Lan can sắt', 'Lan can gỗ', 'Tay vịn'];

export default function RailingWorkerScreen() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedLocation, setSelectedLocation] = useState('Tất cả');
  const [selectedRailingType, setSelectedRailingType] = useState('Tất cả');
  ...
  const filteredWorkers = WORKERS.filter((worker) => {
    ...
    const matchesRailingType =
      selectedRailingType === 'Tất cả' ||
      worker.specialties.some((s) => s.includes(selectedRailingType));

    return matchesSearch && matchesLocation && matchesRailingType;
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Thợ lan can</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Railing Type Filter */}
        <View style={styles.filterSection}>
          <Text style={styles.filterLabel}>Loại lan can</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            {RAILING_TYPES.map((type) => (
              <TouchableOpacity
                key={type}
                style={[
                  styles.filterChip,
                  selectedRailingType === type && styles.filterChipActive,
                ]}
                onPress={() => setSelectedRailingType(type)}
              >
                <Text
                  style={[
                    styles.filterChipText,
                    selectedRailingType === type && styles.filterChipTextActive,
                  ]}
                >
                  {type}
                </Text>
        ...
        <View style={styles.resultsHeader}>
          <Text style={styles.resultsCount}>
            {filteredWorkers.length} thợ lan can
          </Text>
          {(selectedLocation !== 'Tất cả' || selectedRailingType !== 'Tất cả' || searchQuery) && (
        ...
        {filteredWorkers.length > 0 ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>Không tìm thấy thợ lan can</Text>
        ...
        >
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalForm} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.formGroup}>
                <Text style={styles.formLabel}>Loại lan can</Text>
                <TextInput
                  style={styles.formInput}
                  placeholder="VD: Lan can inox cầu thang"
                  value={bookingForm.railingType}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, railingType: text })}
                />
              </View>

              <View style={styles.formGroup}>
                <Text style={styles.formLabel}>Chiều dài (mét)</Text>
                <TextInput
                  style={styles.formInput}
                  placeholder="Nhập chiều dài cần làm"
                  value={bookingForm.length}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, length: text })}
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  style={styles.formInput}
                  placeholder="VD: 100cm"
                  value={bookingForm.height}
                  onChangeText={(text) => setBookingForm({ ...bookingForm, height: text })}
                  keyboardType="numeric"
                />
              </View>
