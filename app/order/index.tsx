...
const orders = [
  {
    ...
    total: 2850000,
    items: [
      { name: 'Gạch lát nền Granite 60x60', qty: 10, price: 185000, image: 'https://via.placeholder.com/80' },
      { name: 'Keo dán gạch Weber', qty: 2, price: 175000, image: 'https://via.placeholder.com/80' },
    ],
  },
  {
    id: 'ORD002',
    ...
    total: 4500000,
    items: [
      { name: 'Bồn cầu TOTO 1 khối', qty: 1, price: 4500000, image: 'https://via.placeholder.com/80' },
    ],
  },
  {
    id: 'ORD003',
    ...
    total: 890000,
    items: [
      { name: 'Sơn nội thất Dulux cao cấp', qty: 1, price: 890000, image: 'https://via.placeholder.com/80' },
    ],
  },
  {
    id: 'ORD004',
    ...
    total: 1200000,
    items: [
      { name: 'Sen vòi Inax cao cấp', qty: 2, price: 600000, image: 'https://via.placeholder.com/80' },
    ],
  },
];
...
const getStatusConfig = (status: string) => {
  switch (status) {
    case 'processing':
      return { label: 'Đang xử lý', color: '#FF9800', bg: '#FFF3E0', icon: 'time' };
    case 'shipping':
      return { label: 'Đang giao', color: '#2196F3', bg: '#E3F2FD', icon: 'car' };
    case 'delivered':
      return { label: 'Đã giao', color: '#4CAF50', bg: '#E8F5E9', icon: 'checkmark-circle' };
    case 'cancelled':
      return { label: 'Đã hủy', color: '#F44336', bg: '#FFEBEE', icon: 'close-circle' };
    default:
      return { label: status, color: '#666', bg: '#f0f0f0', icon: 'help-circle' };
  }
};
    ...
    return (
      <View style={[styles.orderCard, { backgroundColor: cardBg }]}>
        ...
        <View style={styles.orderHeader}>
          <View>
            <Text style={[styles.orderId, { color: textColor }]}>#{item.id}</Text>
        ...
        <View style={styles.orderFooter}>
          <View style={styles.totalRow}>
            ...
            <View style={styles.totalValue}>
              ...
              <Text style={[styles.totalPrice, { color: '#FF6B35' }]}>{formatPrice(item.total)}</Text>
            </View>
          </View>
          ...
          <View style={styles.actionBtns}>
            {item.status === 'delivered' && (
              <>
                ...
                <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#FF6B35' }]}>
                  <Text style={{ color: '#fff', fontWeight: '500' }}>Mua lại</Text>
                </TouchableOpacity>
              </>
            )}
            {item.status === 'shipping' && (
              ...
              >
                <Text style={{ color: '#2196F3', fontWeight: '500' }}>Theo dõi</Text>
              </TouchableOpacity>
            )}
            {item.status === 'processing' && (
              <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#FFEBEE' }]}>
                <Text style={{ color: '#F44336', fontWeight: '500' }}>Hủy đơn</Text>
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  productRow: { flexDirection: 'row', padding: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
