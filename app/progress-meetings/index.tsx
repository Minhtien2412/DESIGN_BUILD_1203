...
export default function ProgressTrackingScreen() {
  ...
  const getMeetingTypeIcon = (type: Meeting['type']) => {
    switch (type) {
      ...
      case 'site-inspection': return 'construct';
            ...
            <View style={styles.meetingList}>
              {filteredMeetings.map(meeting => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <View style={[styles.iconContainer, { backgroundColor: getMeetingStatusColor(meeting.status) + '20' }]}>
                    <Ionicons
                  ...
                  <View style={styles.meetingContent}>
                    <View style={styles.titleRow}>
                      <Text style={[styles.meetingTitle, { color: textColor }]} numberOfLines={1}>
                        {meeting.title}
                      </Text>
                      <View style={[styles.statusDot, { backgroundColor: getMeetingStatusColor(meeting.status) }]} />
                    </View>
                    ...
                    <View style={styles.participantsSummary}>
                      <Text style={[styles.participantsCount, { color: mutedColor }]}>
                        {meeting.participants.length} người tham gia
                      </Text>
                      {meeting.status === 'in-progress' && (
                        <>
                          <Text style={[styles.separator, { color: mutedColor }]}>•</Text>
                          <Text style={[styles.arrivedCount, { color: '#10B981' }]}>
                            {meeting.participants.filter(p => p.status === 'arrived').length} đã tới
                          </Text>
                        </>
                      )}
                    </View>
