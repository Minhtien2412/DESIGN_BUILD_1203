...
const materials = [
  ...
  },
  {
    ...
    features: ['Chống trầy', 'Chống bám bẩn', 'Dễ lau chùi', 'Kháng axit', 'Chịu lực cao'],
    rating: 4.9,
    reviews: 312,
  },
];
...
export default function MaterialCompareScreen() {
  ...
  const [selectedMaterials, setSelectedMaterials] = useState<string[]>(['1', '2']);

  const formatPrice = (price: number) => price.toLocaleString('vi-VN') + 'đ';

  const comparedMaterials = materials.filter((m) => selectedMaterials.includes(m.id));
  ...
  return (
    <View style={[styles.container, { backgroundColor }]}>
      <Stack.Screen options={{ title: 'So sánh vật liệu', headerShown: true }} />
      
      <ScrollView showsVerticalScrollIndicator={false}>
        {/* Material Selector */}
        <View style={styles.selectorSection}>
          <Text style={[styles.sectionTitle, { color: textColor }]}>Chọn vật liệu so sánh (tối đa 3)</Text>
            ...
            {materials.map((material) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  selectedMaterials.includes(material.id) && styles.selectorCardActive,
                ]}
              ...
              >
                ...
                <Text style={[styles.selectorName, { color: textColor }]} numberOfLines={1}>
                  {material.name}
                </Text>
        ...
        <View style={[styles.comparisonHeader, { backgroundColor: cardBg }]}>
          <View style={styles.headerLabel}>
            <Text style={[styles.headerLabelText, { color: textColor }]}>Sản phẩm</Text>
          </View>
        ...
        {/* Specs Comparison */}
        <Text style={[styles.sectionTitle, { color: textColor, marginTop: 20, marginLeft: 16 }]}>
          Thông số kỹ thuật
        </Text>
        {allSpecs.map((spec) => (
          <View key={spec} style={[styles.comparisonRow, { backgroundColor: cardBg }]}>
            <View style={styles.rowLabel}>
        ...
        {allFeatures.map((feature) => (
          <View key={feature} style={[styles.comparisonRow, { backgroundColor: cardBg }]}>
            <View style={styles.rowLabel}>
        ...
        <View style={styles.cartSection}>
          {comparedMaterials.map((material) => (
            <TouchableOpacity key={material.id} style={styles.addToCartBtn}>
              ...
              <Text style={styles.addToCartText}>{material.name.split(' ').slice(0, 2).join(' ')}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
