...
export default function CRMInvoicesScreen() {
  const { invoices, stats, loading, error, refresh, updateInvoice } =
    useInvoices();
  ...
  const handleMarkAsPaid = async (invoiceId: string) => {
    Alert.alert("Xác nhận thanh toán", "Đánh dấu hóa đơn này đã thanh toán?", [
      ...
      <View style={styles.statsHeader}>
        ...
        <View style={styles.dataSourceBadge}>
          ...
          <Text style={styles.dataSourceText}>
            {invoices.length > 0 ? "Live CRM" : "Demo"}
          </Text>
        </View>
      </View>
    ...
    >
      {[
        { key: "all" as FilterType, label: "Tất cả", count: invoices.length },
        {
          ...
          label: "Chưa TT",
          count: invoices.filter((i) => ["1", "3", "6"].includes(i.status))
            .length,
        },
        {
          key: "paid" as FilterType,
          label: "Đã TT",
          count: invoices.filter((i) => i.status === "2").length,
        },
        {
          key: "overdue" as FilterType,
          label: "Quá hạn",
          count: invoices.filter((i) => i.status === "4").length,
        },
      ].map((filter) => (
        <TouchableOpacity
          key={filter.key}
                  ...
                  <View
                    style={[
                      styles.detailStatus,
                      {
                        backgroundColor:
                          (STATUS_CONFIG[
                            selectedInvoice.status as keyof typeof STATUS_CONFIG
                          ]?.color || "#6B7280") + "20",
                      },
                    ]}
                  >
