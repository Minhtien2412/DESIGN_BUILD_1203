...
function ProjectsContent() {
  const { projects, stats, loading, error, refresh, search } = useProjects();
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={[styles.emptyContainer, { flex: 1 }]}>
          ...
          <Text style={[styles.emptyText, { marginTop: 16, color: "#EF4444" }]}>
            Lỗi tải dữ liệu
          </Text>
          <TouchableOpacity onPress={handleRefresh} style={{ marginTop: 12 }}>
            <Text style={{ color: "#0D9488", fontWeight: "600" }}>Thử lại</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
            ...
            <Text
              style={[
                ...
                (selectedStatus === item.value ||
                  (item.value === null && selectedStatus === null)) &&
    ...
    <TouchableOpacity style={styles.projectCard} activeOpacity={0.7}>
      <View style={styles.projectHeader}>
        ...
        <Text style={styles.projectCustomer}>
          <Ionicons name="person-outline" size={12} color="#6B7280" /> Khách
          hàng #{project.clientid || "N/A"}
        </Text>
      </View>
...
function getProgressColor(progress: number): string {
  if (progress >= 80) return "#0D9488";
  if (progress >= 50) return "#0D9488";
  if (progress >= 25) return "#0D9488";
  return "#000000";
}
