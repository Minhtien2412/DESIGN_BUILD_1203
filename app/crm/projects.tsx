...
import { PerfexSyncProvider, usePerfexProjects, usePerfexSync } from '@/context/PerfexSyncContext';
import { Project, formatDate, formatVND, getProjectStatusName } from '@/services/perfexSync';
...
function ProjectsContent() {
  const { projects, isLoading } = usePerfexProjects();
  const { refresh } = usePerfexSync();
  ...
  const stats = useMemo(() => {
    const totalValue = projects.reduce((sum, p) => 
      sum + parseFloat(p.project_cost || '0'), 0
    );
    const inProgress = projects.filter(p => p.status === '2').length;
    const completed = projects.filter(p => p.status === '4').length;
    
    return { totalValue, inProgress, completed, total: projects.length };
  }, [projects]);
    ...
    <TouchableOpacity style={styles.projectCard} activeOpacity={0.7}>
      <View style={styles.projectHeader}>
        ...
        <Text style={styles.projectCustomer}>
          <Ionicons name="person-outline" size={12} color="#6B7280" /> {project.company}
        </Text>
      </View>
