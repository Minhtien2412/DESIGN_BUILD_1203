...
export default function CustomerProjectsScreen() {
    ...
    try {
      ...
      if (
        ...
        mappedCustomers.forEach((customer) => {
          const custProjects = mappedProjects.filter(
            (p) => p.customerId === customer.id,
          );
          customer.totalProjects = custProjects.length;
          customer.activeProjects = custProjects.filter(
            (p) => p.status === "IN_PROGRESS" || p.status === "PLANNING",
          ).length;
          customer.completedProjects = custProjects.filter(
            (p) => p.status === "COMPLETED",
          ).length;
          customer.totalValue = custProjects.reduce(
        ...
        setDataSource("crm");
        console.log(
          `✅ Loaded ${mappedCustomers.length} customers, ${mappedProjects.length} projects from CRM`,
        );
      } else {
      ...
      <View style={styles.customerHeader}>
        <Image
          source={{
            uri:
              item.avatar ||
              "https://ui-avatars.com/api/?name=KH&size=50&background=FF6B35&color=fff",
          }}
  ...
  const renderProjectCard = ({ item }: { item: CustomerProject }) => {
    const statusConfig =
      PROJECT_STATUS[item.status as keyof typeof PROJECT_STATUS];
      ...
      >
        ...
        <Image
          source={{
            uri:
              item.coverImage ||
              "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=400&h=200&q=80",
          }}
      ...
      >
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder={
              viewMode === "customers" ? "Tìm khách hàng..." : "Tìm dự án..."
            }
            ...
            <Image
              source={{
                uri:
                  ...
                  "https://ui-avatars.com/api/?name=KH&size=40&background=FF6B35&color=fff",
              }}
