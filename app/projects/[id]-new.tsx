...
import { constructionDesignerRoute, constructionPaymentProgressRoute } from '@/utils/routes';
...
export default function ProjectDetailScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'progress' | 'team' | 'documents'>('overview');
  ...
  if (loading && !project) {
    return (
      <SafeAreaView style={[styles.centerContainer, { backgroundColor: background }]}>
        ...
        <Text style={[styles.loadingText, { color: textMuted }]}>Đang tải dự án...</Text>
      </SafeAreaView>
    );
  }
  ...
  const statusConfig = STATUS_CONFIG[project.status as ProjectStatus] || STATUS_CONFIG.planning;
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: background }]}>
      ...
      {/* Custom Header */}
      <View style={[styles.customHeader, { backgroundColor: surface, borderBottomColor: border }]}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <View style={[styles.headerStatusBadge, { backgroundColor: `${statusConfig.color}15` }]}>
            <Ionicons name={statusConfig.icon} size={12} color={statusConfig.color} />
            <Text style={[styles.headerStatusText, { color: statusConfig.color }]}>{statusConfig.label}</Text>
          </View>
        </View>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroSection}>
          {project.images && project.images.length > 0 ? (
            <Image source={{ uri: project.images[0] }} style={styles.heroImage} />
          ) : (
            <View style={[styles.heroPlaceholder, { backgroundColor: `${tint}20` }]}>
              <Ionicons name="business-outline" size={64} color={tint} />
            </View>
          )}
          <View style={styles.heroOverlay}>
            <View style={[styles.progressRing, { borderColor: `${statusConfig.color}40` }]}>
              <Text style={[styles.progressRingText, { color: statusConfig.color }]}>{project.progress || 0}%</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: text }]}>Truy cập nhanh</Text>
          <QuickActionCard
            icon="cash-outline"
        ...
        {/* Project Info */}
        <View style={[styles.section, { backgroundColor: surface, borderRadius: 12, padding: 16 }]}>
          <Text style={[styles.sectionTitle, { color: text }]}>Thông tin dự án</Text>
          
          {project.description && (
            <View style={styles.infoRow}>
              <Ionicons name="information-circle-outline" size={20} color={tint} />
              <Text style={[styles.infoText, { color: text }]}>{project.description}</Text>
            </View>
          )}
          ...
          {project.location && (
            <View style={styles.infoRow}>
              ...
              <Text style={[styles.infoText, { color: text }]}>{project.location}</Text>
            </View>
          )}
          ...
          {project.start_date && (
            <View style={styles.infoRow}>
              ...
              <Text style={[styles.infoText, { color: text }]}>
                ...
                {project.end_date && ` • Kết thúc: ${formatDate(project.end_date)}`}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={[styles.section, { backgroundColor: surface, borderRadius: 12, padding: 16 }]}>
          <View style={styles.progressHeader}>
            <Text style={[styles.sectionTitle, { color: text }]}>Tiến độ tổng thể</Text>
            <Text style={[styles.progressPercentage, { color: statusConfig.color }]}>{project.progress || 0}%</Text>
          </View>
          <View style={[styles.progressTrack, { backgroundColor: `${statusConfig.color}20` }]}>
            <View
              style={[
                styles.progressFill,
                { backgroundColor: statusConfig.color, width: `${project.progress || 0}%` },
              ]}
            />
          </View>
          <View style={styles.progressMilestones}>
            ...
            <View style={styles.milestone}>
              <View style={[styles.milestoneMarker, { backgroundColor: statusConfig.color }]} />
              <Text style={[styles.milestoneText, { color: textMuted }]}>Tầng trệt</Text>
            </View>
            <View style={styles.milestone}>
              <View style={[styles.milestoneMarker, { backgroundColor: '#D1D5DB' }]} />
              <Text style={[styles.milestoneText, { color: textMuted }]}>Hoàn thiện</Text>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  errorText: { marginTop: 16, fontSize: 16, fontWeight: '600', marginBottom: 16 },
  ...
  headerStatusBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 12 },
  ...
  quickActionIconBox: { width: 48, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  ...
  progressTrack: { height: 12, borderRadius: 6, overflow: 'hidden', marginBottom: 16 },
