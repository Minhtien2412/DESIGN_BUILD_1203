...
import { changePassword, updateUser, uploadAvatar } from "@/services/userApi";
import {
...
export default function EditProfileScreen() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Upload avatar separately if changed (avatar is a local URI starting with file://)
      if (
        avatarUri &&
        ...
        const uploadResult = await uploadAvatar(avatarUri);
        if (uploadResult.success && uploadResult.data?.avatar_url) {
          newAvatarUrl = uploadResult.data.avatar_url;
          console.log("✅ Avatar uploaded:", newAvatarUrl);
        } else {
          console.warn("⚠️ Avatar upload failed:", uploadResult.message);
          // Continue with profile update even if avatar upload fails
        }
      }

      // Update user profile WITHOUT avatar field (backend doesn't accept it)
      const updatedUser = await updateUser(Number(user.id), {
        name: name.trim(),
        phone: phone.trim() || undefined,
        // Note: avatar is handled via separate upload endpoint
      });
      ...
      await editProfile({
        ...
        avatar: newAvatarUrl || updatedUser.avatar || undefined,
      });
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Lỗi",
        e?.message || "Không thể đổi mật khẩu. Kiểm tra mật khẩu cũ.",
      );
    } finally {
  ...
  return (
    <ThemedView style={{ flex: 1 }}>
      <Stack.Screen
        options={{ title: "Cài đặt tài khoản", headerBackTitle: "Quay lại" }}
      />
      ...
      >
          ...
          <View style={styles.field}>
            ...
            <View
              style={[
                ...
                { borderColor: border, backgroundColor: "#f5f5f5" },
              ]}
            >
        ...
        <View
          style={[
            styles.infoCard,
            { backgroundColor: "#f0f9ff", marginTop: 24 },
          ]}
        >
          <Ionicons
