...
import { CreditScoreIndicator, PointsBalanceCard, PointsConversionModal } from '@/components/ui/PointsWallet';
import { RatingItem, RatingSummaryCard, StarRatingDisplay } from '@/components/ui/StarRating';
import { useProfile } from '@/context/ProfileContext';
import type { AvailabilityStatus, RoleTheme, UserProfileFull } from '@/types/profile';
...
function AvailabilityBadge({ status, message }: AvailabilityBadgeProps) {
  const getStatusInfo = () => {
    switch (status) {
      case 'available':
        return { label: 'Sẵn sàng', color: '#4CAF50', icon: 'checkmark-circle' };
      case 'busy':
  ...
  return (
    <View style={[styles.availabilityBadge, { backgroundColor: `${info.color}15` }]}>
      <View style={[styles.statusDot, { backgroundColor: info.color }]} />
      <Text style={[styles.availabilityText, { color: info.color }]}>{info.label}</Text>
    </View>
  );
}
...
function ProfileHeader({ profile, theme, onEditCover, onEditAvatar, isOwnProfile }: ProfileHeaderProps) {
  return (
    <View style={styles.headerContainer}>
      ...
      <View style={styles.profileInfoContainer}>
        <View style={styles.avatarWrapper}>
          ...
          {isOwnProfile && (
            <TouchableOpacity style={styles.editAvatarBtn} onPress={onEditAvatar}>
        ...
        <View style={styles.nameContainer}>
          ...
          {profile.yearsExperience && (
            <Text style={styles.experience}>• {profile.yearsExperience} năm KN</Text>
          )}
        </View>
        ...
        <View style={styles.statsRow}>
          ...
          <Text style={styles.statText}>{profile.stats.completedJobs} việc</Text>
        </View>
...
function QuickActions({ profile, theme, isOwnProfile, onMessage, onCall, onShare, onEdit }: QuickActionsProps) {
  if (isOwnProfile) {
    return (
    ...
    <View style={styles.sectionCard}>
      ...
      {profile.availability.location && (
        <View style={styles.infoRow}>
          ...
          <View style={styles.infoContent}>
            <Text style={styles.infoText}>{profile.availability.location.address}</Text>
      ...
      <View style={styles.infoRow}>
        ...
        <Text style={styles.infoText}>
          Phản hồi trong <Text style={{ color: '#4CAF50', fontWeight: '600' }}>
