/**
 * Meetings Index Screen - Unified Meeting Hub
 * Gộp từ meet/index.tsx + meeting/index.tsx
 * Clean black & white premium experience với đầy đủ chức năng
 */
...
export default function MeetingsIndexScreen() {
  ...
  const handleShareMeeting = async (m: ScheduledMeeting) => {
    try {
      await Share.share({ message: getMeetingShareText(m) });
    } catch {
      await Clipboard.setStringAsync(m.meetingLink);
      Alert.alert("Đã sao chép liên kết");
    }
  };

  const isLive = (s: ScheduledMeetingStatus) =>
    s === "IN_PROGRESS" || s === "STARTED";
    ...
    return (
      <Animated.View entering={FadeInUp.delay(delay).springify()}>
        ...
        >
          {btnLoading ? (
            <ActivityIndicator
              color={primary ? colors.bg : colors.accent}
              size="small"
            />
          ) : (
            <Ionicons
        ...
        <AnimatedPressable
          style={[
            styles.meetingCard,
            {
              ...
              borderColor: live ? colors.live : colors.border,
        ...
        >
          <View style={styles.cardContent}>
            <View
              style={[
                styles.meetingIcon,
                { backgroundColor: live ? colors.live : colors.accentSoft },
              ]}
            >
              <Ionicons
                name="videocam"
                size={20}
                color={live ? "#fff" : colors.accent}
              />
            </View>
            <View style={styles.meetingInfo}>
              ...
              <Text
                style={[styles.meetingTime, { color: colors.textSecondary }]}
              >
                {formatDateTime(item.scheduledStartTime)} · {item.duration} phút
              </Text>
              <View style={styles.metaRow}>
                ...
                <Text
                  style={[styles.participantCount, { color: colors.textMuted }]}
                >
            ...
            <View style={styles.cardActions}>
              <Pressable
                ...
                onPress={() => handleShareMeeting(item)}
              >
                <Ionicons
                  name="share-outline"
                  size={18}
      ...
      >
        ...
        >
          ...
          >
            ...
            />
            <View style={styles.statItem}>
              <Text style={[styles.statNumber, { color: colors.live }]}>
                {meetings.filter((m) => isLive(m.status)).length}
              </Text>
              <Text style={[styles.statLabel, { color: colors.textMuted }]}>
                Đang diễn ra
              </Text>
          ...
          {loading ? (
            ...
            >
              ...
              <Text style={[styles.emptySubtitle, { color: colors.textMuted }]}>
                Tạo cuộc họp mới hoặc tham gia bằng mã
              </Text>
              <Pressable
            ...
            <View style={styles.meetingsList}>
              {meetings.map((m, i) => (
                <MeetingCard key={m.id} item={m} index={i} />
              ))}
            </View>
          )}
        </View>
