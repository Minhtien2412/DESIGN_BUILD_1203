...
export default function CommunicationsScreen() {
  ...
  const getCallStatusText = (status: string, isIncoming: boolean) => {
    switch (status) {
      case 'missed': return isIncoming ? 'Nhớ cuộc gọi' : 'Không trả lời';
      case 'completed': return isIncoming ? 'Cuộc gọi đến' : 'Cuộc gọi đi';
      ...
      <View style={styles.contentContainer}>
        ...
        <View style={styles.messageRow}>
          <Text
            style={[
              styles.lastMessage,
              !item.lastMessage.isRead && !item.lastMessage.isFromMe && styles.unreadMessage,
            ]}
            numberOfLines={1}
          >
            {item.lastMessage.isFromMe && 'Bạn: '}
            {item.lastMessage.type === 'text' ? item.lastMessage.content : '📷 Hình ảnh'}
          </Text>
          {item.unreadCount > 0 && (
    ...
    >
      ...
      <View style={styles.contentContainer}>
        ...
        <View style={styles.callRow}>
          ...
          />
          <Text style={[styles.callStatus, { color: getCallStatusColor(item.status) }]}>
            {getCallStatusText(item.status, item.isIncoming)}
          </Text>
          <Text style={styles.callDuration}> • {formatCallDuration(item.duration)}</Text>
        </View>
      </View>
  ...
  const renderFooter = () => {
    const isLoadingMore = activeTab === 'messages' ? messagesLoadingMore : callsLoadingMore;
  ...
  const isRefreshing = activeTab === 'messages' ? messagesRefreshing : callsRefreshing;
