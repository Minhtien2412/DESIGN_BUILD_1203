...
const promotions = [
  {
    ...
    minOrder: 500000,
    expiry: '31/01/2026',
    image: 'https://via.placeholder.com/400x200/FF6B35/fff?text=Dulux+20%25',
    category: 'paint',
    used: 1250,
    remaining: 250,
  },
  {
    ...
    minOrder: 1000000,
    expiry: '15/01/2026',
    image: 'https://via.placeholder.com/400x200/4CAF50/fff?text=Free+Ship',
    category: 'shipping',
    used: 3200,
    remaining: 800,
  },
  {
    ...
    minOrder: 5000000,
    expiry: '28/02/2026',
    image: 'https://via.placeholder.com/400x200/2196F3/fff?text=500K+Off',
    category: 'sanitary',
    used: 456,
    remaining: 544,
  },
  ...
  const renderPromotion = ({ item }: { item: typeof promotions[0] }) => (
    <TouchableOpacity style={[styles.promoCard, { backgroundColor: cardBg }]}>
      ...
      <View style={styles.promoContent}>
        ...
        <Text style={[styles.promoTitle, { color: textColor }]}>{item.title}</Text>
        <Text style={styles.promoDesc}>{item.description}</Text>
        
        {item.minOrder > 0 && (
          <Text style={styles.minOrder}>Đơn tối thiểu: {formatPrice(item.minOrder)}</Text>
        )}
        ...
        <View style={styles.progressContainer}>
          <View style={[styles.progressBar, { width: `${(item.used / (item.used + item.remaining)) * 100}%` }]} />
        </View>
...
const styles = StyleSheet.create({
  ...
  promoTitle: { fontSize: 18, fontWeight: '600', marginBottom: 6, marginTop: 8 },
  promoDesc: { color: '#666', fontSize: 14, marginBottom: 8 },
  minOrder: { color: '#999', fontSize: 13, marginBottom: 12 },
  promoFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  ...
  progressContainer: { height: 4, backgroundColor: '#f0f0f0', borderRadius: 2, marginBottom: 4 },
