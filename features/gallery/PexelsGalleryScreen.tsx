...
export default function PexelsGalleryScreen() {
  ...
  const modalPlayer = useVideoPlayer(
    selectedVideo ? pexelsService.getSDVideoUrl(selectedVideo) : null,
    (player) => {
      player.loop = true;
    },
  );
  ...
  const loadPhotos = useCallback(
    async (pageNum = 1, refresh = false) => {
      ...
      setLoading(true);
      try {
        const response = await pexelsService.getConstructionPhotos(
          selectedCategory,
          pageNum,
          20,
        );
  ...
  const loadVideos = useCallback(
    async (pageNum = 1, refresh = false) => {
      ...
      setLoading(true);
      try {
        const response = await pexelsService.getConstructionVideos(
          selectedCategory,
          pageNum,
          15,
        );
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      >
        ...
        <View style={styles.headerCenter}>
          <Image
            source={{
              uri: "https://images.pexels.com/lib/api/pexels-white.png",
            }}
