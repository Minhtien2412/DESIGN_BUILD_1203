...
async function syncToBackend(event: string, data: any) {
  try {
    const response = await fetch(`${BACKEND_URL}/strapi-sync/products`, {
      method: "POST",
      headers: {
    ...
    console.log(`[Strapi->BE] Product ${event}: ${data.id} - Status: ${response.status}`);
  } catch (error) {
    console.error(`[Strapi->BE] Error syncing product:`, error);
  }
}
