...
export interface ProgressWebSocketContextType {
  ...
  disconnect: () => void;
  subscribeToTask: (taskId: number, callback: (data: any) => void) => () => void;
...
const ProgressWebSocketContext = createContext<ProgressWebSocketContextType | undefined>(undefined);
...
export function ProgressWebSocketProvider({
  ...
  const connect = useCallback(async () => {
    ...
    try {
      setConnecting(true);
      setError(null);
      console.log('[ProgressWebSocket] Attempting to connect to /progress namespace...');
      ...
      console.log('[ProgressWebSocket] Connection failed (graceful fallback):', errorMessage);
  ...
  const subscribeToTask = useCallback(
    (taskId: number, callback: (data: any) => void): (() => void) => {
      if (!socket || !connected) {
        console.warn('[ProgressWebSocket] Not connected, cannot subscribe to task:', taskId);
  ...
  const subscribeToProject = useCallback(
    (projectId: number, callback: (data: any) => void): (() => void) => {
      if (!socket || !connected) {
        console.warn('[ProgressWebSocket] Not connected, cannot subscribe to project:', projectId);
        return () => {}; // Return no-op cleanup
      }
  ...
  useEffect(() => {
    ...
    if (user) {
      console.log('[ProgressWebSocket] User logged in, auto-connecting...');
      connect();
    } else {
...
export function useProgressWebSocket(): ProgressWebSocketContextType {
  ...
  if (!context) {
    throw new Error('useProgressWebSocket must be used within ProgressWebSocketProvider');
  }
  return context;
}
