...
> = {
  ...
  "payment-success": {
    ...
    priority: "high",
    imageUrl:
      "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=300&h=150&q=80",
    primaryAction: {
      label: "Xem hóa đơn",
      action: "open",
    },
...
export function RichNotification({
  ...
  const getColorScheme = (scheme?: string) => {
    switch (scheme) {
      case "blue":
        return { primary: "#007AFF", bg: "#E8F2FF" };
      case "green":
        return { primary: "#34C759", bg: "#E8F8EA" };
      case "orange":
        return { primary: "#FF9500", bg: "#FFF4E8" };
      case "red":
        return { primary: "#FF3B30", bg: "#FFE8E8" };
      case "purple":
        return { primary: "#AF52DE", bg: "#F3E8FF" };
      default:
        return { primary: "#8E8E93", bg: "#F8F8F8" };
    }
  };
            ...
            <View style={styles.content}>
              ...
              {template.subtitle && (
                <Text style={styles.subtitle}>
                  {interpolateText(template.subtitle)}
                </Text>
              )}
              {template.body && (
                <Text style={styles.body}>
                  {interpolateText(template.body)}
                </Text>
              )}
            </View>
            {renderActions()}
              ...
              <View style={styles.progressContainer}>
                ...
                {template.progress.showPercentage && (
                  <Text style={styles.progressText}>
                    {Math.round(
                      (template.progress.current / template.progress.total) *
                        100,
                    )}
                    %
                  </Text>
      ...
      <View style={styles.actions}>
        {template.secondaryAction && (
          <Pressable
            ...
            onPress={() =>
              onAction?.("secondary", template.secondaryAction?.route)
            }
          >
...
export function createNotificationFromTemplate(
  ...
  return {
    ...
    route: template.primaryAction?.route?.replace(
      /\[([^\]]+)\]/g,
      (match, key) => String(data[key.toLowerCase()] || match),
    ),
    meta: {
      templateId,
      template,
