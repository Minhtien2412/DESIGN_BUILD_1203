...
// Icon mapping
const NOTIFICATION_ICONS: Record<NotificationType, { name: string; color: string; bg: string }> = {
...
export function NotificationToast({ notification, onPress, onDismiss, duration = 4000 }: ToastProps) {
  ...
  return (
    ...
    >
      <Pressable style={styles.toastContent} onPress={onPress}>
        {notification.avatar ? (
          <Image source={{ uri: notification.avatar }} style={styles.toastAvatar} />
        ) : (
        ...
        <View style={styles.toastBody}>
          ...
          <Text style={styles.toastMessage} numberOfLines={2}>{notification.body}</Text>
        </View>
...
export function NotificationItem({ item, onPress, onDelete }: NotificationItemProps) {
    ...
    <View style={styles.itemWrapper}>
      ...
      <Animated.View
        style={[
          ...
          { backgroundColor: item.isRead ? background : surface, transform: [{ translateX: swipeAnim }] },
        ]}
      >
        <Pressable style={styles.itemContent} onPress={() => onPress?.(item)}>
          ...
          <View style={styles.itemBody}>
            <View style={styles.itemHeader}>
              <Text style={[styles.itemTitle, { color: text }, !item.isRead && styles.itemTitleUnread]} numberOfLines={1}>
                {item.title}
              </Text>
            ...
            <Text style={[styles.itemMessage, { color: textMuted }]} numberOfLines={2}>
              {item.body}
            </Text>
          </View>
...
export function NotificationCenter({
  ...
  const FilterTab = ({ type, label }: { type: NotificationType | 'all'; label: string }) => (
    <Pressable
      style={[
        styles.filterTab,
    ...
    >
      <Text style={[styles.filterTabText, filter === type && { color: '#fff' }]}>
        {label}
      </Text>
    </Pressable>
  );
  ...
  const renderEmpty = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={[styles.emptyTitle, { color: text }]}>Không có thông báo</Text>
